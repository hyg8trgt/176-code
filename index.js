...
$(function (){
    $(".input_field").keyup(function(){
        ...
        // console.log(input_number.ep(input_number))
...
function displayStory(){
    ...
    for(var i=0;i<story.words.length;i++){
        var html=`<button class="word_bank">${story.words[i]}</button>`
        $("#bank_words").append(html)
    }
    $("#input_fields").empty();
    for(var i=0;i<story.inputs;i++){
        var input_html=`<input type="text" class="input_field" id="input_${i}" placeholder="Input_${i+1}"></input>`
